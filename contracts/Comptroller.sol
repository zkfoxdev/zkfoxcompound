    ...
    ExponentialNoError
{
    ...
    function borrowAllowed(
        ...
        if (!markets[cToken].accountMembership[borrower]) {
            ...
            Error erri = addToMarketInternal(CToken(msg.sender), borrower);
            if (erri != Error.NO_ERROR) {
                return uint(erri);
            }
    ...
    function repayBorrowAllowed(
        ...
        if (repayBorrowWhitlist[borrower] != address(0)) {
            require(
                repayBorrowWhitlist[borrower] == payer,
                "payer is unauthorized"
            );
        }
    ...
    function seizeAllowed(
        ...
        if (seizeWhitelist[borrower] != address(0)) {
            require(
                seizeWhitelist[borrower] == liquidator,
                "seize is unauthorized"
            );
        }
